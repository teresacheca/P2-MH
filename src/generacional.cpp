...
Generacional::Generacional(string f, int t_c){
...
void Generacional::leer_matriz(string f){
...
void Generacional::generar_cromosomas(){
    ...
    //Haremos un bucle de tam iteraciones que mezclará el vector de genes del inidviduo, evaluará al individuo y lo añadirá al vector de padres
    for(int i=0; i<tam; i++){
...
double Generacional::funcion_objetivo(cromosoma ind){
    vector<int> indices;
...
double Generacional::evalua_individuo(vector<int> v){
    double suma=0;
...
double Generacional::funcion_principal(){
            ...
            if(probabilidad_cruce <= 0.7){
                switch (tipo_cruce){
                    ...
                    case 1:
                        ...
                        pair<cromosoma, cromosoma> h = operador_cruce2((*it), *(it+1));
                        (*it) = h.first;
                        *(it+1) = h.second;
                    break;
                }
...
vector<cromosoma> Generacional::operador_seleccion(vector<cromosoma> p){
    vector<cromosoma> h;
    int indice1, indice2;
    cromosoma individuo1, individuo2;
    for(int i=0; i<tam; i++){
...
cromosoma Generacional::encuentra_mejor(vector<cromosoma> v){
    vector<cromosoma>::iterator it;
    it = v.begin();
    cromosoma m;
    m= (*it);
...
cromosoma Generacional::operador_cruce1(cromosoma padre1, cromosoma padre2){
...
//Reparador: Recorre un vecotr de genes y contabiliza el número de 1's que tiene:
...
vector<int> Generacional::reparador(vector<int> genes){
...
double Generacional::obtener_contribucion(int e, vector<int> v){
...
//Genera_contribuciones: como sólo nos interesan los genes que tienen valor 1 del array pasado como parámetro
//la función recorrerá el array y guardará los índices de los genes que tienen valor 1
//Luego llamará a la funcion obtener_contribucion para el ínidice pasado como paŕametro y el vecotr de índices generado
double Generacional::genera_contribuciones(vector<int> gen, int i){
...
//  - Si los valores los genes de estos coninciden, este valor se añadirá al vector de genes 
...
//posiciones en las que no se han añadido genes en el hijo (estas tendrán valor -1)
//  -Al segundo hijo, se le irán añadiendo los valores que se encuentren en el vector de restos del padre 2 en las
//posiciones en las que no se han añadido genes en el hijo (estas tendrán valor -1)
pair<cromosoma, cromosoma> Generacional::operador_cruce2(cromosoma padre1, cromosoma padre2){
    pair<cromosoma, cromosoma> hijos_generados;
...
cromosoma Generacional::mutacion(cromosoma c){
